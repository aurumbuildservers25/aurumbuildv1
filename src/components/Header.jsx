        ...
        <div className="max-w-screen-xl mx-auto px-6 h-full relative flex items-center justify-between">
          ...
          {isHomePage && (
            <div className="hidden lg:flex absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 items-center gap-8">
              <button type="button" onClick={() => handleSectionClick('home')} className={`font-medium transition-all duration-300 rounded-full px-4 py-2 ${currentSection === 'home' ? 'font-semibold shadow-lg' : ''}`} style={{ backgroundColor: currentSection === 'home' ? (division === 'industrial' ? '#FFB833' : '#D9B566') : 'transparent', color: currentSection === 'home' ? (division === 'industrial' ? '#0C0E14' : '#24324B') : (division === 'industrial' ? '#E2E8F0' : '#24324B') }} aria-current={currentSection === 'home' ? 'page' : undefined}>{nav.home}</button>
              <button type="button" onClick={() => handleSectionClick('about')} className={`font-medium transition-colors duration-300`} style={{ color: currentSection === 'about' ? (division === 'industrial' ? '#FFB833' : '#D9B566') : (division === 'industrial' ? '#E2E8F0' : '#24324B') }}>{nav.about}</button>
              <button type="button" onClick={() => handleSectionClick('technology')} className={`font-medium transition-colors duration-300`} style={{ color: currentSection === 'technology' ? (division === 'industrial' ? '#FFB833' : '#D9B566') : (division === 'industrial' ? '#E2E8F0' : '#24324B') }}>{nav.technology}</button>
              <button type="button" onClick={() => handleSectionClick('projects')} className={`font-medium transition-colors duration-300`} style={{ color: currentSection === 'projects' ? (division === 'industrial' ? '#FFB833' : '#D9B566') : (division === 'industrial' ? '#E2E8F0' : '#24324B') }}>{nav.projects}</button>
              <Link to={createPageUrl('contact')} className={`font-medium transition-colors duration-300`} style={{ color: isContactPage ? (division === 'industrial' ? '#FFB833' : '#D9B566') : (division === 'industrial' ? '#E2E8F0' : '#24324B') }}>{nav.contact}</Link>
          ...
          <div className="flex items-center gap-2">
            {!isHomePage && (
              <Button asChild size="sm" className={`font-semibold shadow-lg text-sm hidden sm:inline-flex`} style={{ backgroundColor: division === 'industrial' ? '#FFFFFF' : '#24324B', color: division === 'industrial' ? '#0C0E14' : '#F5F3F0' }} onMouseEnter={(e) => e.currentTarget.style.backgroundColor = division === 'industrial' ? '#E2E8F0' : '#1E293B'} onMouseLeave={(e) => e.currentTarget.style.backgroundColor = division === 'industrial' ? '#FFFFFF' : '#24324B'}>
                <Link to={createPageUrl('Home')} onClick={handleGoHome}>{nav.home}</Link>
              </Button>
            )}
